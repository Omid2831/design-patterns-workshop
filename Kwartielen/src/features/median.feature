Feature: Median lifespan of battery brands
  As a consumer researcher
  I want to calculate the median lifespan of two battery brands
  So that I can understand the middle value of their performance

  Background:
    Given a battery dataset "Brand A" with values 86,87,94,95,98,99,100,100,102,103,106,107,108,110,110,112,112,112,115,115
    And a battery dataset "Brand B" with values 55,82,86,94,95,100,101,103,105,106,107,109,110,113,113,116,117,119,124,132

  Scenario: Calculate median for Brand A
    When I calculate the median for "Brand A"
    Then the result should be 104.5

  Scenario: Calculate median for Brand B
    When I calculate the median for "Brand B"
    Then the result should be 106.5
